...
def has_nearby_tourism_context(rows: list[dict[str, object]], current_index: int) -> bool:
    start_index = max(0, current_index - 10)
    end_index = min(len(rows), current_index + 11)
    ...
    for neighbor_index in range(start_index, end_index):
        ...
        if contains_keyword(neighbor_row.get("poi"), "旅游景点") or truthy(neighbor_row.get("app_travel")):
            return True
...
def process_hotel_scene(user_df: pl.DataFrame) -> pl.DataFrame:
    ...
    while index < row_count:
        row = rows[index]
        if labels[index] != "" or not contains_keyword(row.get("poi"), "酒店旅馆"):
            index += 1
            continue
        if not has_nearby_tourism_context(rows, index):
            index += 1
            continue
        ...
        while scan_index < row_count:
            ...
            if labels[scan_index] != "":
                break
            if not contains_keyword(scan_row.get("poi"), "酒店旅馆"):
                break
            if not has_nearby_tourism_context(rows, scan_index):
                break

            segment_end = scan_index
            scan_index += 1

        labels[index] = HOTEL_CHECKIN_LABEL
        for label_index in range(index + 1, segment_end + 1):
            if labels[label_index] == "":
